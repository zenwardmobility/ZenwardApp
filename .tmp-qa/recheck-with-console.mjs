...
async function newChromeSession(port) {
  const chrome = spawn(
    ...
    ["--headless=new", `--remote-debugging-port=${port}`, "--no-first-run", "--no-default-browser-check", "--disable-gpu",
     `--user-data-dir=/private/tmp/claude-501/-Users-datamatics-ZenWard/9c849e26-8432-486d-8d64-3dcf5894ae96/scratchpad/recheck-console-chrome-${port}`],
  ...
  ws.addEventListener("message", (event) => {
    ...
    if (msg.method === "Runtime.exceptionThrown" && msg.params.sessionId === sessionId) {
      console.log(`[EXCEPTION] ${JSON.stringify(msg.params.exceptionDetails.exception?.description || msg.params.exceptionDetails)}`);
    }
  });
  return {
...
async function run() {
  const s = await newChromeSession(9991);
