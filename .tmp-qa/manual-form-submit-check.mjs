...
async function newChromeSession(port) {
  const chrome = spawn(
    ...
    ["--headless=new", `--remote-debugging-port=${port}`, "--no-first-run", "--no-default-browser-check", "--disable-gpu",
     `--user-data-dir=/private/tmp/claude-501/-Users-datamatics-ZenWard/9c849e26-8432-486d-8d64-3dcf5894ae96/scratchpad/manual-form-${port}-${Date.now()}`],
...
async function run() {
  const s = await newChromeSession(9986);
  ...
  await sleep(2000);
  console.log("path before manual submit:", await s.currentPath());
  await fillField(s, 'input[name="fullName"]', "Recovered Owner Name");
  await fillField(s, 'input[name="businessName"]', "Recovered Business Name");
  await s.evaluate(`document.querySelector('form').requestSubmit(); true;`);
  await sleep(2000);
  console.log("path after manual submit:", await s.currentPath());
  s.close();
  process.exit(0);
}
