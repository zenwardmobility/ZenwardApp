...
async function newChromeSession(port) {
  const chrome = spawn(
    ...
    ["--headless=new", `--remote-debugging-port=${port}`, "--no-first-run", "--no-default-browser-check", "--disable-gpu",
     `--user-data-dir=/private/tmp/claude-501/-Users-datamatics-ZenWard/9c849e26-8432-486d-8d64-3dcf5894ae96/scratchpad/recheck-chrome-${port}`],
...
async function run() {
  const s = await newChromeSession(9990);
  ...
  // Poll path every 300ms for up to 8s, logging every change.
  let last = null;
  const start = Date.now();
  while (Date.now() - start < 8000) {
    const p = await s.currentPath();
    if (p !== last) {
      console.log(`t+${Date.now() - start}ms: ${p}`);
      last = p;
    }
    await sleep(200);
  }
