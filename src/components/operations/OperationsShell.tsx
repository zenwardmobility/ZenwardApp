...
export function OperationsShell({ sidebar, header, children }: OperationsShellProps) {
  return (
    <>
      <div className="flex h-dvh flex-col items-center justify-center bg-surface-app px-lg text-center md:hidden">
        <EmptyState
      ...
      <div className="hidden h-dvh bg-surface-app md:flex">
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          <AppHeader {...header} />
          <main className="flex-1 overflow-y-auto px-4 py-lg lg:px-6 lg:py-xl">{children}</main>
        </div>
      </div>
    </>
  );
